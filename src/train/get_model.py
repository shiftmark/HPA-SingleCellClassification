...
class GetModel:
    ...
    def set_backbone(self, backbone_name: str, include_top=True, **kwargs):
        ...
        Args:
            backbone_name (str): The name of the backbone to use. Supported names: all from 'tf.keras.applications'.
                Please check documentation: https://www.tensorflow.org/api_docs/python/tf/keras/applications .
            include_top (bool): If True, the backbone will include it's default top (head).
            **kwargs: The remaining keyword arguments from the backbone. See documentation mentioned above.
        Returns:
        ...
        if callable(attributes):
            ...
            msg = f"The backbone name ({backbone_name}) is not valid." \
                  f"Please check the tf.kera.applications documentation."
    ...
    def add_top(self, n_classes: int, dropout: float, nodes: int):
        ...
        Args:
            ...
            nodes (int): The number of neurons in the dense layer.
        ...
        if not self.include_top:
            ...
            msg = f"The backbone already has a top." \
                  f"Set 'include_top=False' when calling 'set_backbone', then call 'add_top'."
            raise(ValueError(msg))
